...
CMD node -e "require('http').createServer((req, res) => res.end('Alive')).listen(10000);" & openclaw setup && openclaw config set gateway.mode local && openclaw config set channels.telegram.dmPolicy allowlist && openclaw config set channels.telegram.allowFrom '["5658658059"]' && openclaw gateway run
